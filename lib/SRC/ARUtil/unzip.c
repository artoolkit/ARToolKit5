    ...
    while (pfile_in_zip_read_info->stream.avail_out>0)
    {
        if (pfile_in_zip_read_info->stream.avail_in == 0)
        {
            ...
            if (pfile_in_zip_read_info->stream.next_in != NULL)
                bytes_not_read = (uInt)(pfile_in_zip_read_info->read_buffer + UNZ_BUFSIZE - pfile_in_zip_read_info->stream.next_in);
            ...
            while (total_bytes_read != bytes_to_read)
            {
                ...
                bytes_read = (uInt)(ZREAD64(pfile_in_zip_read_info->z_filefunc, pfile_in_zip_read_info->filestream,
